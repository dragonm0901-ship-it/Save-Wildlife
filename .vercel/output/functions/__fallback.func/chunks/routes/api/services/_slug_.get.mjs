...
const _slug__get = defineEventHandler(async (event) => {
  ...
  try {
    ...
    const { data, error } = await client.from("services").select("*").eq("slug", slug).single();
    if (!error && data) {
      const service2 = data;
      return {
        ...service2,
        image: service2.image_url,
        highlights: typeof service2.highlights === "string" ? JSON.parse(service2.highlights) : service2.highlights
      };
    }
  } catch (_err) {
  }
